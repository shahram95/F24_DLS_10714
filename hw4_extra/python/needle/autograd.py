...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node_i in reverse_topo_order:
        # Sum accumulated gradients for current node
        node_i.grad = sum_node_list(node_to_output_grads_list[node_i])
        
        # Skip gradient computation for leaf nodes (no operation)
        if node_i.op is None:
            continue
            
        # Get gradients with respect to inputs of current node
        input_grads = node_i.op.gradient_as_tuple(node_i.grad, node_i)
        
        # Distribute gradients to input nodes
        for input_node, input_grad in zip(node_i.inputs, input_grads):
            # Initialize gradient list for input node if needed
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # Recursively visit all input nodes first
    for input_node in node.inputs:
        topo_sort_dfs(input_node, visited, topo_order)
    
    # Add current node after all inputs are processed
