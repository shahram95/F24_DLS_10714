...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        current_size = prod(self._shape)
        new_size = prod(new_shape)
        
        # Verify shape compatibility
        if current_size != new_size:
            raise ValueError(
                f"Cannot reshape array of size {current_size} into shape {new_shape} "
                f"which has size {new_size}"
            )
        
        # Verify memory layout is compatible
        if not self.is_compact():
            raise ValueError(
                "Reshape requires a compact array. Current array has non-contiguous memory layout."
            )
        
        # Create new array view with updated shape and corresponding strides
        return NDArray.make(
            shape=new_shape,
    ...
    def permute(self, new_axes):
        ...
        if len(new_axes) != len(self._shape):
            raise ValueError(
                f"Permutation length {len(new_axes)} does not match array dimensions {len(self._shape)}"
            )
        if sorted(new_axes) != list(range(len(self._shape))):
            raise ValueError(
                f"Invalid permutation {new_axes}. Must contain each dimension exactly once."
            )
        
        # Convert axes to list for numpy indexing
        axes_list = list(new_axes)
        
        # Create new shape and strides based on permutation
        permuted_shape = tuple(self._shape[i] for i in axes_list)
        permuted_strides = tuple(self._strides[i] for i in axes_list)
        
        # Return new view with permuted dimensions
        return NDArray.make(
    ...
    def broadcast_to(self, new_shape):
        ...
        # Validate shape compatibility and prepare new strides
        broadcast_strides = list(self._strides)
        for curr_dim, new_dim, curr_stride, idx in zip(
            self._shape, 
            new_shape, 
            self._strides, 
            range(len(self._shape))
        ):
            ...
            if curr_dim != new_dim:
                if curr_dim != 1:
                    raise ValueError(
                        f"Cannot broadcast shape {self._shape} to {new_shape}: "
                        f"Dimension {idx} must be 1 or equal to {new_dim}, got {curr_dim}"
                    )
                # Set stride to 0 for broadcast dimension
                broadcast_strides[idx] = 0
        
        # Create new view with broadcast shape
        return NDArray.make(
            shape=new_shape,
            strides=tuple(broadcast_strides),
            device=self._device,
            handle=self._handle
        )
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        # Calculate new shape based on slice ranges and steps
        new_shape = [
            (sl.stop - sl.start + sl.step - 1) // sl.step 
            for sl in idxs
        ]
        
        # Calculate offset from start positions
        dimension_offsets = [
            sl.start * stride 
            for sl, stride in zip(idxs, self._strides)
        ]
        total_offset = sum(dimension_offsets)
        
        # Calculate new strides accounting for step sizes
        new_strides = tuple(
            stride * sl.step 
            for stride, sl in zip(self._strides, idxs)
        )
        
        # Create view with updated parameters
        return NDArray.make(
            shape=new_shape,
            strides=new_strides,
            device=self._device,
    ...
    def flip(self, axes):
        ...
        ### BEGIN YOUR SOLUTION
        # Validate axes
        if not all(0 <= axis < len(self.shape) for axis in axes):
            raise ValueError(
                f"Invalid axes {axes} for array with {len(self.shape)} dimensions"
            )
        
        # Create modified strides for flipping
        modified_strides = list(self.strides)
        for flip_axis in axes:
            modified_strides[flip_axis] = -modified_strides[flip_axis]
        
        # Calculate offset for flipped dimensions
        cumulative_offset = sum(
            (self.shape[axis] - 1) * self.strides[axis]
            for axis in axes
        )
        
        # Create flipped view and compact it
        flipped_array = NDArray.make(
            shape=self.shape,
            strides=tuple(modified_strides),
            device=self._device,
            handle=self._handle,
            offset=cumulative_offset
        )
        
        # Return compacted version to ensure proper memory layout
    ...
    def pad(self, axes):
        ...
        if len(axes) != len(self.shape):
            raise ValueError(
                f"Padding specification length {len(axes)} doesn't match array "
                f"dimensions {len(self.shape)}"
            )
        
        # Calculate new shape after padding
        padded_shape = tuple(
            original_size + left_pad + right_pad
            for (left_pad, right_pad), original_size in zip(axes, self.shape)
        )
        
        # Create new zero-filled array of padded size
        padded_array = self.device.full(
            shape=padded_shape,
            fill_value=0
        )
        
        # Define slices for inserting original data
        insertion_slices = tuple(
            slice(left_pad, left_pad + original_size)
            for (left_pad, _), original_size in zip(axes, self.shape)
        )
        
        # Insert original data into padded array
        padded_array[insertion_slices] = self
